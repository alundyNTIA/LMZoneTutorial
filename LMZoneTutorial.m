...
TxLat = [ ...
    24.55478848, 24.5700912438, 24.57031318, ... % Original TXs
    38.85000, ...   % ~50 km North
    38.71820, ...   % ~50 km NE
    38.00000, ...   % ~50 km SE
    37.95000, ...   % ~50 km South
    38.00000, ...   % ~50 km SW
    38.71820, ...   % ~50 km NW
    38.40000  ...   % ~50 km East/West adjusted below
    ];

TxLon = [ ...
    -81.80757429, -81.77097236, -81.73660619, ... % Original TXs
    -77.20000, ...  % North
    -76.57000, ...  % NE
    -76.64000, ...  % SE
    -77.20000, ...  % South
...
TxHtm = [ ...
    20.0, 20.0, 20.0, ... % Original TX heights
...
% DLL pairs multiple interfering tx with one rx in this scenario
% Creates an array of the same rx
RxLatAry = RxLat*ones(1,num_interferers);
RxLonAry = RxLon*ones(1,num_interferers);
RxHtmAry = RxHtm*ones(1,num_interferers);
